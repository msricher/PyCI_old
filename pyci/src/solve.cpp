...
SparseOp_t::SparseOp_t(const Ham &ham, const WfnType &wfn, const int_t rows, const int_t cols)
...
#pragma omp parallel
  {
    ...
    SparseOp_t op(end - start, ncol);
    std::vector<uint_t> det(wfn.get_nword());
    std::vector<int_t> occs(wfn.get_nocc());
    std::vector<int_t> virs(wfn.get_nvir());
...
double SparseOp_t::get_element(const int_t i, const int_t j) const {
...
void SparseOp_t::perform_op(const double *x, double *y) const {
  int_t nthread = omp_get_max_threads();
  int_t chunksize = nrow / nthread + ((nrow % nthread) ? 1 : 0);
#pragma omp parallel
  {
...
void SparseOp_t::perform_op_cepa0(const double *x, double *y, const int_t refind) const {
  int_t nthread = omp_get_max_threads();
  int_t chunksize = nrow / nthread + ((nrow % nthread) ? 1 : 0);
#pragma omp parallel
  {
...
void SparseOp_t::perform_op_transpose_cepa0(const double *x, double *y, const int_t refind) const {
  int_t i;
  for (i = 0; i < ncol; ++i)
    y[i] = 0;
...
void SparseOp_t::rhs_cepa0(double *b, const int_t refind) const {
...
void SparseOp_t::init_thread_sort_row(const int_t idet) {
...
void SparseOp_t::init_thread_condense(SparseOp_t &op, const int_t ithread) {
  int_t i, start;
  if (!ithread) {
    op.data.swap(data);
...
void SparseOp_t::init_thread_add_row(const Ham &ham, const DOCIWfn &wfn, const int_t idet,
...
void SparseOp_t::init_thread_add_row(const Ham &ham, const FullCIWfn &wfn, const int_t idet,
...
void SparseOp_t::init_thread_add_row(const Ham &ham, const GenCIWfn &wfn, const int_t idet,
                                     uint_t *det, int_t *occs, int_t *virs) {
